...
import { BarChart3, TrendingUp, Users, Share2, Eye, Calendar, ArrowUpRight, ArrowDownRight, Instagram, Facebook } from "lucide-react";
...
export const SocialAnalytics = () => {
    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            // Fetch queue platform breakdown
            const { data: queue } = await supabase
                .from("social_post_queue")
                .select("platform, status");

            // Fetch media source breakdown
            ...
            queue?.forEach(q => {
                if (q.platform === 'instagram') breakdown.instagram++;
                if (q.platform === 'facebook') breakdown.facebook++;
            });

            setStats({
                totalPosts: posts?.length || 0,
                publishedCount: queue?.filter(q => q.status === 'sent').length || 0,
                scheduledCount: queue?.filter(q => q.status === 'pending').length || 0,
        ...
        <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden relative">
            ...
            <CardContent>
                ...
                <div className="mt-2 flex items-center gap-1">
                    ...
                    <span className={`text-xs font-bold ${trend === 'up' ? 'text-emerald-500' : 'text-rose-500'}`}>{trendValue}</span>
        ...
        <div className="space-y-8 mt-6 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm">
                <div>
                    ...
                    <p className="text-slate-500 font-medium mt-1">Track your content performance across social platforms.</p>
                </div>
                <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                    <button className="px-4 py-1.5 text-xs font-bold bg-white dark:bg-slate-700 rounded-md shadow-sm">30 Days</button>
                    <button className="px-4 py-1.5 text-xs font-bold text-slate-500">90 Days</button>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title="Total Posts" value={stats.totalPosts} icon={Megaphone} trend="up" trendValue="+12%" />
                <StatCard title="Total Published" value={stats.publishedCount} icon={Share2} trend="up" trendValue="+8%" />
                <StatCard title="Scheduled" value={stats.scheduledCount} icon={Calendar} trend="down" trendValue="-3%" />
                <StatCard title="WhatsApp Media" value={stats.whatsappImages} icon={Facebook} trend="up" trendValue="+24%" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <CardContent className="space-y-6">
                        <div className="space-y-2">
                            <div className="flex justify-between items-center text-sm">
                                <span className="flex items-center gap-2 font-bold"><Instagram className="w-4 h-4 text-pink-500" /> Instagram</span>
                            ...
                            <div className="h-3 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                                <div
                                    ...
                                    style={{ width: `${stats.platformBreakdown.instagram + stats.platformBreakdown.facebook > 0 ? (stats.platformBreakdown.instagram / (stats.platformBreakdown.instagram + stats.platformBreakdown.facebook)) * 100 : 0}%` }}
                                />
                            </div>
                        </div>
                        <div className="space-y-2">
                            <div className="flex justify-between items-center text-sm">
                                <span className="flex items-center gap-2 font-bold"><Facebook className="w-4 h-4 text-blue-600" /> Facebook</span>
                            ...
                            <div className="h-3 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                                <div
                                    ...
                                    style={{ width: `${stats.platformBreakdown.instagram + stats.platformBreakdown.facebook > 0 ? (stats.platformBreakdown.facebook / (stats.platformBreakdown.instagram + stats.platformBreakdown.facebook)) * 100 : 0}%` }}
                                />
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
                    ...
                    <CardContent className="h-48 flex items-end gap-3 pb-2">
                        {[40, 65, 45, 80, 55, 90, 75, 60, 85, 95, 100].map((h, i) => (
                            <div key={i} className="flex-1 flex flex-col items-center gap-2">
                                ...
                                <span className="text-[10px] text-slate-400 font-mono italic">M{i + 1}</span>
                            </div>
                        ))}
                    </CardContent>
