...
import { PLANS, PRICING_FAQS, type Plan } from '@/lib/constants/pricing';
...
function PricingCard({ plan, billingCycle }: { plan: Plan; billingCycle: 'monthly' | 'annual' }) {
  const priceValue = typeof plan.price.monthly === 'string'
    ? plan.price.monthly
    : (billingCycle === 'monthly' ? plan.price.monthly : plan.price.annual);

  return (
    <div
      className={`relative group p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] border transition-all duration-500 ${
        plan.highlight
          ? 'bg-white/[0.05] border-[#00F0FF]/30 shadow-[0_0_50px_rgba(0,240,255,0.1)]'
      ...
      <div className="mb-8">
        <div className="flex items-baseline gap-1">
          <span className="text-4xl font-black">
            {typeof plan.price.monthly === 'string' ? priceValue : `\u20B9${priceValue}`}
          </span>
          {typeof plan.price.monthly !== 'string' && <span className="text-gray-500">/mo</span>}
        </div>
        {typeof plan.price.monthly !== 'string' && billingCycle === 'annual' && (
...
export default function PricingPageContent() {
  ...
  return (
    <div className="text-white selection:bg-[#00F0FF]/30 selection:text-white relative overflow-x-hidden">
      {/* Hero Section */}
