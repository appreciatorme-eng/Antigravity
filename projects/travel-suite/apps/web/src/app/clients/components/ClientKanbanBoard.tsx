        ...
        <div className="space-y-5">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <div className="flex-1 sm:w-72 flex items-center gap-2 px-4 min-h-[44px] rounded-xl bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 shadow-sm">
                        <Search className="w-4 h-4 text-text-muted shrink-0" />
                        <input
                            type="text"
                            placeholder="Search clients..."
                            className="flex-1 bg-transparent border-none focus:ring-0 text-gray-900 dark:text-white placeholder:text-text-muted/60 text-sm font-medium outline-none"
                            value={searchTerm}
            ...
            <div className="p-4">
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                        <button
                            ...
                            className="min-h-[44px] min-w-[44px] flex items-center justify-center text-gray-600 dark:text-gray-400 hover:text-primary transition-colors bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 hover:border-primary/30"
                        ...
                        <Link
                            ...
                            className="min-h-[44px] min-w-[44px] flex items-center justify-center text-gray-600 dark:text-gray-400 hover:text-blue-600 transition-colors bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 hover:border-blue-300"
                ...
                <div className="mt-3 flex flex-wrap items-center gap-1.5">
                    {client.client_tag && client.client_tag !== "standard" && (
                        <span className={cn(
                            "text-[11px] font-black px-1.5 py-0.5 rounded-md border uppercase tracking-wider",
                    ...
                    <span className="flex items-center gap-0.5 text-[11px] font-black px-1.5 py-0.5 rounded-md border border-emerald-200 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:border-emerald-800 dark:text-emerald-400 uppercase tracking-wider">
                        <IndianRupee className="w-2.5 h-2.5" />
                        {formatINR((client.trips_count || 1) * (client.budget_max || 85000))}
                    </span>
                    {client.preferred_destination && (
                        <span className="text-[11px] font-medium text-gray-600 dark:text-gray-400 truncate max-w-[100px]">
                            {"\uD83D\uDCCD"} {client.preferred_destination}
                        </span>
                    )}
                </div>
                ...
                <div className="mt-3 pt-3 border-t border-gray-100 dark:border-slate-800/80 flex items-center justify-between">
                    <div className="flex gap-1.5">
                        <button
                            ...
                            className="min-w-[44px] min-h-[44px] rounded-lg bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 flex items-center justify-center text-gray-600 dark:text-gray-400 hover:text-primary hover:border-primary/30 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                    ...
                    {client.trips_count ? (
                        <span className="flex items-center gap-1 text-[11px] font-black text-primary bg-primary/8 px-2 py-1 rounded-lg border border-primary/15 uppercase tracking-wider">
                            <Plane className="w-3 h-3" />
                            {client.trips_count} trip{client.trips_count > 1 ? "s" : ""}
                        </span>
                    ) : (
                        <span className="text-[11px] text-gray-500 dark:text-gray-400 font-medium">No trips yet</span>
                    )}
                </div>
            </div>
        </div>
