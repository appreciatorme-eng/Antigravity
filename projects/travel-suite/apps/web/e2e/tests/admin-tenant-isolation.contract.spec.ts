...
test.describe('Admin tenant isolation - unauthenticated', () => {
  ...
  test('blocks cost overview endpoint', async ({ request }) => {
    const response = await request.get(`/api/admin/cost/overview?organization_id=${FOREIGN_ORG_ID}`);
    expect(response.status()).toBe(401);
  });
...
authTest.describe('Admin tenant isolation - non-admin users', () => {
  authTest('forbids command center access', async ({ clientPage }) => {
    const response = await clientPage.request.get('/api/admin/operations/command-center');
...
authTest.describe('Admin tenant isolation - scoped access', () => {
  authTest('non-super-admin cannot force foreign organization scope', async ({ adminPage }) => {
  ...
  authTest('cost cap mutation is restricted to super admins', async ({ adminPage }) => {
    const response = await adminPage.request.post('/api/admin/cost/overview', {
      data: {
        category: 'amadeus',
        capUsd: 42,
      },
