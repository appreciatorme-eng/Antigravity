...
function MarketingAssetStatusBadge({ state }: { state: string }) {
  const config: Record<string, { label: string; color: string; icon: React.ElementType }> =
    {
      asset_generated: { label: "Auto-Generated", color: "#10b981", icon: Sparkles },
      queued_for_review: { label: "Scheduled", color: "#8b5cf6", icon: Calendar },
      published: { label: "Published", color: "#059669", icon: CheckCheck },
    };
...
export function ReviewCard({
  ...
  const hasAutoGeneratedAsset = review.marketing_asset_state === "asset_generated";
  const hasScheduledOrPublishedAsset =
    review.marketing_asset_state === "queued_for_review" ||
    review.marketing_asset_state === "published";
  const marketingLabel = hasScheduledOrPublishedAsset
    ? "Refresh Asset"
    : hasAutoGeneratedAsset
    ? "Already Generated"
    : "Generate Asset";
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {review.marketing_asset_state && (
            <MarketingAssetStatusBadge state={review.marketing_asset_state} />
          )}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canCreateMarketingAsset && onGenerateAsset && (
            <button
              ...
              disabled={isGeneratingAsset || hasAutoGeneratedAsset}
              className="text-xs text-gray-400 hover:text-violet-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1 transition-colors"
              title={hasAutoGeneratedAsset ? "Asset already auto-generated" : undefined}
            >
