...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: LucideIconComponent }> = {
    ...
    ready:      { label: "In Review", color: "bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300",      icon: CheckCircle2 },
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                    ...
                    >
                        {f === "all"
                          ? `${FILTER_LABELS[f]} (${posts.length})`
                          : `${FILTER_LABELS[f]} (${posts.filter(p => p.status === f).length})`}
                    </button>
                ))}
            </div>
                                ...
                                <div className="p-4 flex-1 flex flex-col gap-3">
                                    ...
                                    <div className="flex gap-2 pt-1 border-t border-slate-100 dark:border-slate-800">
                                        ...
                                        {(post.status === "draft" || post.status === "ready") && (
                                            <a
                                                href="/social"
                                                className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-bold rounded-lg bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors"
                                            >
                                                <Send className="w-3.5 h-3.5" /> Open Studio
                                            </a>
                                        )}
                                        <button
