...
async function fetchUnsplashImage(query: string, ms = 5000): Promise<string | null> {
    ...
    try {
        ...
        if (!url) return null;
        return url.replace("https://images.unsplash.com/", "/unsplash-img/");
    } catch {
        clearTimeout(tid);
        return null;
    }
...
async function fetchPexelsImage(query: string, ms = 5000): Promise<string | null> {
...
async function fetchPixabayImage(query: string, ms = 5000): Promise<string | null> {
...
async function fetchStockFallback(query: string): Promise<{ url: string; source: string } | null> {
...
async function resolveActivityImage({
    ...
    if (!transferLike) {
        for (const candidate of entityQueries) {
            const entity = await searchWikidataEntity(candidate, location, destinationText);
            if (!entity) continue;
            ...
            if (entity.title) {
                const wikiThumb = await fetchWikiThumbnail(entity.title);
                if (wikiThumb) {
                    return {
                        url: wikiThumb,
    ...
    for (const candidate of searchQueries.slice(0, 3)) {
        const result = await fetchStockFallback(candidate);
        if (result) {
            return {
                url: result.url,
    ...
    const destinationFallback = await fetchStockFallback(destinationQuery);
    ...
    const fallback = getContextualFallback(`${title} ${destinationText}`) ?? getDeterministicFallback(destinationText || title);
    return {
        url: fallback,
        source: "fallback",
        confidence: "low",
