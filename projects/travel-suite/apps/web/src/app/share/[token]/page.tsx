...
function hasDuplicateAutoGeneratedActivityImages(itinerary: ItineraryResult): boolean {
    const counts = new Map<string, number>();

    for (const day of itinerary.days ?? []) {
        for (const activity of day.activities ?? []) {
            const image = activity.image || activity.imageUrl;
            if (!image || !isAutoGeneratedImageUrl(image)) continue;

            const normalized = image.trim().split("?")[0]?.toLowerCase();
...
export default async function SharedTripPage({
    ...
    const fullTripDataRecord = fullTripData as unknown as Record<string, unknown>;
    const rawImageSearchVersion =
        typeof fullTripDataRecord.image_search_version === "number"
            ? Number(fullTripDataRecord.image_search_version)
            : 0;
    const needsImages = fullTripData.days?.some(d =>
        d.activities?.some(a => !a.image && !a.imageUrl)
    ) ?? false;
    const hasDuplicateAutoImages = hasDuplicateAutoGeneratedActivityImages(fullTripData);
    const shouldRefreshAutoGeneratedImages =
        (
            rawImageSearchVersion < IMAGE_SEARCH_VERSION
            || hasDuplicateAutoImages
        ) &&
        (fullTripData.days?.some((day) =>
            day.activities?.some((activity) => isAutoGeneratedImageUrl(activity.image || activity.imageUrl))
        ) ?? false);

    if (needsImages || shouldRefreshAutoGeneratedImages) {
        const enriched = await populateItineraryImages(
            fullTripData as ItineraryResult & Record<string, unknown>,
