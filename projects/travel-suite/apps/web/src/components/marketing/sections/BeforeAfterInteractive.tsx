...
export function BeforeAfterInteractive({ oldWayItems, newWayItems }: BeforeAfterInteractiveProps) {
  ...
  return (
    <div 
      ref={containerRef}
      className="relative w-full max-w-5xl mx-auto min-h-[500px] rounded-[40px] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] border border-white/5 select-none"
    >
      ...
      <div className="absolute inset-0 bg-[#0A0A0A] p-8 md:p-12 flex flex-col justify-center overflow-hidden">
        ...
        <div className="w-full absolute inset-0 flex items-center pr-8 md:pr-16 pl-[55%]">
          <div className="max-w-md">
            <div className="flex items-center gap-4 mb-10">
              <motion.div 
                ...
                className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#00F0FF]/30 to-[#A259FF]/30 border border-[#00F0FF]/50 flex items-center justify-center text-[#00F0FF] shadow-[0_0_30px_rgba(0,240,255,0.3)]"
              >
                <CheckCircle2 size={32} />
              </motion.div>
              <h3 className="text-4xl font-black bg-gradient-to-r from-[#00F0FF] to-[#A259FF] bg-clip-text text-transparent drop-shadow-[0_0_15px_rgba(0,240,255,0.4)]">
                TravelBuilt
              </h3>
            </div>
            <div className="space-y-8">
              {newWayItems.map((item, i) => (
                <motion.div 
                  ...
                  key={i} 
                  className="flex items-start gap-5 text-gray-100 group"
                >
                  <span className="text-2xl mt-1 text-[#00F0FF] drop-shadow-[0_0_10px_rgba(0,240,255,0.8)] group-hover:scale-125 transition-transform">{item.icon}</span>
                  <p className="text-lg font-semibold leading-relaxed tracking-wide">{item.text}</p>
                </motion.div>
              ))}
            </div>
          </div>
      ...
      >
        ...
        <div className="w-full absolute inset-0 flex items-center pl-8 md:pl-16 pr-[55%]">
          <div className="max-w-md">
            <div className="flex items-center gap-4 mb-10">
              <div className="w-16 h-16 rounded-2xl bg-neutral-800 border border-neutral-700 flex items-center justify-center text-red-500 shadow-inner">
                <XCircle size={32} />
              </div>
              <h3 className="text-3xl font-bold text-neutral-400">The Old Way</h3>
            </div>
            <div className="space-y-8">
              {oldWayItems.map((item, i) => (
                <div key={i} className="flex items-start gap-5 text-neutral-500 opacity-80">
                  <span className="text-2xl mt-1 grayscale opacity-50">{item.icon}</span>
      ...
      {containerWidth > 0 && (
        ...
        >
          ...
          {/* Scanner Knob */}
          <motion.div 
            className={`relative w-14 h-14 bg-black border-2 border-[#00F0FF] rounded-full flex items-center justify-center shadow-[0_0_30px_#00F0FF] backdrop-blur-md transition-transform duration-300 ${isDragging ? 'scale-125' : 'scale-100 group-hover:scale-110'}`}
          >
